...
namespace Algorum.Strategy.SupportResistance
{
   ...
   /// </summary>
   public class GapUpStrategy : QuantEngineClient
   {
      private class State
      {
         public bool Bought;
      ...
      public static async Task<GapUpStrategy> GetInstanceAsync( string url, string apiKey, StrategyLaunchMode launchMode, string sid )
      {
         var strategy = new GapUpStrategy( url, apiKey, launchMode, sid );
         await strategy.InitializeAsync();
         return strategy;
      }

      private GapUpStrategy( string url, string apiKey, StrategyLaunchMode launchMode, string sid )
      ...
      public override async Task OnTickAsync( TickData tickData )
      {
         try
         {
            ...
            {
               await LogAsync( LogLevel.Debug, $"{tickData.Timestamp}, {tickData.LTP}, yh {yesterdayHigh}, yl {yesterdayLow}, yc {yesterdayClose}, to {todayOpen}" );
               ...
               ( !_state.Bought ) && ( string.IsNullOrWhiteSpace( _state.CurrentOrderId ) ) )
            {
               ...
               var log = $"{tickData.Timestamp}, Placed sell (short) order for {qty} units of {_symbol.Ticker} at price (approx) {tickData.LTP}, {tickData.Timestamp}";
               await LogAsync( LogLevel.Information, log );
            ...
            else if ( _state.CurrentOrder != null )
            {
                  ...
                  ( _state.Bought ) )
               {
                  ...
                  var log = $"{tickData.Timestamp}, Placed buy (short) order for {qty} units of {_symbol.Ticker} at price (approx) {tickData.LTP}, {tickData.Timestamp}";
                  await LogAsync( LogLevel.Information, log );
